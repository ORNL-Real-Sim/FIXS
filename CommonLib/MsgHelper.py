...
class MsgHelper:
    ...
    def set_traffic_light_message_field(self, traffic_light_msg_field: List[str]):
        for field in traffic_light_msg_field:
            self.traffic_light_msg_field_valid[field] = True

    def set_detector_message_field(self, detector_msg_field: List[str]):
        for field in detector_msg_field:
            self.detector_msg_field_valid[field] = True
    ...
    def pack_veh_data(self, byte_data: bytearray, byte_index, veh_data: VehData):
        ...
        msg_size = (
            round(self.vehicle_msg_field_valid.get('id', 0) * (1 + len(veh_data.id))  # id
                  + self.vehicle_msg_field_valid.get('type', 0) * (1 + len(veh_data.type))  # type
                  + self.vehicle_msg_field_valid.get('vehicleClass', 0) * (1 + len(veh_data.vehicleClass))  # vehicleClass
                  ...
                  + self.vehicle_msg_field_valid.get('linkId', 0) * (1 + len(veh_data.linkId))  # linkId
                  ...
                  + self.vehicle_msg_field_valid.get('signalLightId', 0) * (1 + len(veh_data.signalLightId))  # signalLightId
    ...
    def pack_traffic_light_data(self, byte_data: bytearray, byte_index, traffic_light_data: TrafficLightData):
        ...
        msg_size = (
            round(self.traffic_light_msg_field_valid.get('id', 0) * 4  # id
                  + self.traffic_light_msg_field_valid.get('name', 0) * (1 + len(traffic_light_data.name))  # name
                  + self.traffic_light_msg_field_valid.get('state', 0) * (1 + len(traffic_light_data.state))  # state
            )
        )
        traffic_light_msg_size = round(msg_size) + self.msg_each_header_size
        
        # Pack message size as uint16, 2 bytes
        byte_data[byte_index:byte_index+2] = struct.pack('H', traffic_light_msg_size)
        byte_index += 2
        # Pack message type as uint8, 1 byte
        byte_data[byte_index] = MessageType.traffic_light_data
        byte_index += 1

        # Pack fields conditionally based on traffic_light_msg_field_valid
        if self.traffic_light_msg_field_valid.get('id'):
            byte_data[byte_index:byte_index+4] = struct.pack('I', traffic_light_data.id)
            byte_index += 4

        if self.traffic_light_msg_field_valid.get('name'):
            byte_data, byte_index = MsgHelper.pack_string(byte_data, byte_index, traffic_light_data.name)
    ...
    def pack_detector_data(self, byte_data: bytearray, byte_index, detector_data: DetectorData):
        # Calculate nMsgSize based on detector_msg_field_valid flags and detector_data field lengths
        msg_size = (
            round(self.detector_msg_field_valid.get('id', 0) * 1  # id
                  + self.detector_msg_field_valid.get('name', 0) * (1 + len(detector_data.name))  # name
                  + self.detector_msg_field_valid.get('state', 0) * 1  # state
            )
        )
        detector_msg_size = round(msg_size) + self.msg_each_header_size
        
        # Pack message size as uint16, 2 bytes
        byte_data[byte_index:byte_index+2] = struct.pack('H', detector_msg_size)
        byte_index += 2
        # Pack message type as uint8, 1 byte
        byte_data[byte_index] = MessageType.detector_data
        byte_index += 1

        # Pack fields conditionally based on detector_msg_field_valid
        if self.detector_msg_field_valid.get('id'):
            byte_data[byte_index] = detector_data.id
            byte_index += 1

        if self.detector_msg_field_valid.get('name'):
            byte_data, byte_index = MsgHelper.pack_string(byte_data, byte_index, detector_data.name)

        if self.detector_msg_field_valid.get('state'):
    ...
    def depack_detector_data(self, byte_data: bytes)-> DetectorData:
        ...
        byte_index = 0
        if self.detector_msg_field_valid.get('id'):
            detector_data.id, byte_index = MsgHelper.unpack_uint8(byte_data, byte_index)
        if self.detector_msg_field_valid.get('name'):
            str_data, str_len, byte_index, uint8Arr = MsgHelper.depack_string(byte_data, byte_index)
            detector_data.name = str_data
        if self.detector_msg_field_valid.get('state'):
            detector_data.state, byte_index = MsgHelper.unpack_uint8(byte_data, byte_index)
    ...
    @ staticmethod
    def depack_string(byte_data: bytearray, byte_index: int):
    ...
    def pack_string(byte_data: bytearray, byte_index: int, str_data: str):
        ...
        # str_data = str_data.encode('utf-8')  # Convert string to bytes  
        ...
        byte_data[byte_index:byte_index+str_len] = str_data[:str_len]
        byte_index += str_len
...
def test_pack_simple_message():
    ...
    byte_index = 0
    # Pack data
    packed_data, nMsgSize, byte_index = msg_helper.pack_veh_data(byte_data=byte_data, byte_index=byte_index, veh_data=veh_data)
...
def test_pack_string():
    ...
    byte_index = 0
    str_data = "Hello"
    packed_data, byte_index = msg_helper.pack_string(byte_data, byte_index, str_data)
    print("Packed string:", packed_data[:byte_index])
