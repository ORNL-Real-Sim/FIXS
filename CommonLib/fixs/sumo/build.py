"""Building a run's SUMO scenario. The front door is fixs.sumo.scenario().

Everything here is reached through that one call; the module exists so
fixs/sumo/__init__.py can be the interface rather than 570 lines of
implementation.
...
to have arrived. These answer about FILES, and the point is that they run BEFORE
the stack exists: an app builds its scenario, reports it, and only then connects.
So there is no connection guard, and adding one later to make the namespaces look
alike would break every caller.
"""
...
def scenario(sumocfg, out_dir, *, ego=None, vtypes=None, outputs=None,
    ...
    _report(path)
    return Scenario(path=str(path), built=built)
