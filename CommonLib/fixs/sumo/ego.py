...
def generate(opt):
    """Build the scenario `opt` describes; return (out_cfg, ego_rou, n_edges).

    Split out of main() so `fixs.sumo` can call this without a subprocess and
    without parsing stdout. main() keeps every print, so the command line is
    unchanged for anyone driving it that way.
    """
    ...
    return out_cfg, ego_rou, n_edges


def main(argv=None):
    opt = build_parser().parse_args(argv)
    out_cfg, ego_rou, n_edges = generate(opt)
