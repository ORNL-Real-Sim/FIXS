...
**Python Scripts:**
- `RS_CM<version>_BuildConfig_<dSPACE>.py` (e.g. `RS_CM13_1_3_BuildConfig_2024a.py`) - dSPACE ConfigurationDesk build configuration that compiles CarMaker HIL executables with the Real-Sim libraries. Generated by `scripts/dispatch/5b_carmaker_dspace.ps1` from CarMaker's own `CM_BuildConfig.py`, one per CarMaker / dSPACE version pair in `dependencies.yaml`.
- `patch_user_c.ps1` - Patches the CarMaker project's `User.c` for the Real-Sim integration
...
#### **ProprietaryFiles/**
Private submodule ([ORNL-Real-Sim/ProprietaryFiles](https://github.com/ORNL-Real-Sim/ProprietaryFiles)) with the CarMaker, VISSIM and dSPACE integration code that cannot be public. Without access it stays empty; the release zip ships the binaries built from it.
...
The build is driven by `scripts/dispatch/dispatch.bat`, which initializes the clone
(`scripts/initialize_fixs.ps1`: submodules, native deps, yaml-cpp) and then runs the
numbered component scripts in `scripts/dispatch/` (`1_external_libraries.bat` for
yaml-cpp, `2_core_components.bat` for TrafficLayer, and so on), copying everything to
`build/`. Each component script can also be run on its own. [BUILD.md](BUILD.md) is
the reference for all of it: prerequisites, what each script builds, debug builds,
and the release CI.
...
1. **Install prerequisites** as listed in [BUILD.md](BUILD.md#prerequisites). The
   versions of MATLAB, CarMaker, dSPACE and SUMO come from `dependencies.yaml`.
...
3. **Build:**
   ...
   ```
   Its first step initializes a fresh clone (see
   [First-Run Setup](BUILD.md#first-run-setup-fresh-clone)).

4. **Verify the build:** `build/` holds `TrafficLayer.exe` and the simulator-specific
   DLLs, and `build/BUILD_INFO.txt` records what was built.
...
In dSPACE ConfigurationDesk, run the build configuration script generated for your
CarMaker / dSPACE versions, e.g. `CarMaker/RS_CM13_1_3_BuildConfig_2024a.py` (see
the CarMaker section under Repository Structure).
