...
class SumoEnvMultiAgent:
    ...
    # def start(self, mpr, step_length):
    #     # modify the mpr (e.g., 0.2, 0.4, 0.6, 0.8, 0.999)
    #     self.change_config_directory(mpr, step_length, 'Unsubscription')
    #     self.sumoCmd = [self.sumoBinary, "-c", self.sumo_config_file]
    ...
    #
    #             if veh_id == 'ego':
    #                 if traci.vehicle.getRoadID('ego') == "-2801":
    ...
    #                 cav_eco_speed = cav.get_eco_speed(self.phase_tracking_dict, self.spat_statuses)
    ...
    # def get_cav_control_noncontrol(self, route_edges, next_tls, travel_direction):
    #     # still have two or more intersections to go
    #     if travel_direction in ['EB', 'WB']:
    #         if len(next_tls) >= 2:
    #             control = 'True'
...
if __name__ == "__main__":
    ...
    # with subscription
    for mpr in [0.0, 0.1, 0.2, 0.5, 1.0]:
    # for mpr in [1.0]:
