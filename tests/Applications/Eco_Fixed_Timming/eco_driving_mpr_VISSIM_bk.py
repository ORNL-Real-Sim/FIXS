    ...
    def start_subscription(self, vehicle_dynamics=True, eco_driving=True):
        ...
        while simsec <= self.end_of_simulation - 300:
            ...
            try:
                ...
                simsec = self.vissim.Simulation.SimulationSecond
                print('send to simulink')
                self.socket_helper.sendData(sim_state, simsec, self.socket2simulink)
                self.socket_helper.clear_data()
                # receive data from the client (the actual vehicle data after the vehidle dynamics model)
                self.socket_helper.recv_data(self.socket2simulink)
                simulink_speed_dic = {veh_data.id:veh_data.speedDesired for veh_data in self.socket_helper.vehicle_data_receive_list}
                for idx, veh_data in enumerate(self.socket_helper.vehicle_data_receive_list):
                    if veh_data.id in ori_speed_dic.keys() and veh_data.id in simulink_speed_dic.keys():
                        veh_data_tmp = veh_data
                        speed_desired_simulink = simulink_speed_dic[veh_data.id]
                        veh_data_tmp.speedDesired = speed_desired_simulink
                        self.socket_helper.vehicle_data_send_list.append(veh_data_tmp)
            except Exception as e:
                print('send FIXS simulink done')
...
if __name__ == '__main__':
    experiment_config = {
        'driver_model_path': r"DriverModel_RealSim_v2021.dll",
