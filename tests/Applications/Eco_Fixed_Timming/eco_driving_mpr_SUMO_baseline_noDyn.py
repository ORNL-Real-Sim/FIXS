...
class SumoEnvMultiAgent:
    ...
    def change_config_directory(self):
        file_name = f'{int(self.penetration_rate*100)}%_{int(1/self.step_length)}Hz' + f'{"_with" if self.enable_vehicle_dynamics else "_without"}_baseline_NoDyn'
    ...
    def apply_vehicle_control_FIXS(self, eco_speed_dic, vehicle_dynamics=False, eco_driving=False, control_veh_ids = ['ego']):
        ...
        # 2. 构造原始速度字典
        ori_speed = {
            veh_data.id: veh_data.speed
            for veh_data in self.socket_helper.vehicle_data_receive_list
            if veh_data.id and veh_data.id.strip() != ""
        }

        for veh_id, speed_val in ori_speed.items():
            if speed_val is not None and veh_id in control_veh_ids:
                veh_data = VehData(id=veh_id, speedDesired=speed_val)
        ...
        # # 3. 构造 Simulink 发送列表（只包含 ego 的原始速度）
        # simulink_tx = []
        # if 'ego' in ori_speed_dic:
        #     simulink_tx.append(VehData(id='ego', speedDesired=ori_speed_dic['ego']))

        # # 4. 构造 Traffic Layer 发送列表（排除 ego）
        # self.socket_helper.vehicle_data_send_list = []
        # for veh_id, ori_speed in ori_speed_dic.items():
        #     if veh_id == 'ego' and not eco_driving:
        #         # baseline: 不把 ego 发回 FIXS，避免锁死
        #         continue
        ...
        if self.enable_vehicle_dynamics and hasattr(self, "socket2simulink"):
            try:
                # 临时发送 ego 的速度给 Simulink
                # self.socket_helper.vehicle_data_send_list = simulink_tx
                self.socket_helper.sendData(sim_state, sim_time, self.socket2simulink)
                # 接收 Simulink 返回，但 baseline 下不使用
        ...
        for idx in range(len(self.socket_helper.vehicle_data_send_list)):
            ...
            if not vehicle_dynamics and self.enable_vehicle_dynamics:
            ...
            # else:
                ...
                self.socket_helper.vehicle_data_send_list[idx].speedDesired = -1



                
        # 6. 如果 Traffic Layer 列表为空，补一个空条目
        if len(self.socket_helper.vehicle_data_send_list) == 0:
            veh_data = VehData()
            self.socket_helper.vehicle_data_send_list.append(veh_data)

        # 7. 打印并发送到 Traffic Layer
        try:
            for veh in self.socket_helper.vehicle_data_send_list:
                print(f"Sending to FIXS: Vehicle ID: {veh.id}, Speed Desired: {veh.speedDesired}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--workingDirectory", type=str, help="Specify working directory", default='MPR_6')
