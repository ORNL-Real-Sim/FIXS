...
% function realsim_script_dspace(settingDir)
settingDir = pwd;
...
RealSimPara.speedInit = 22.35; % Initial speed of the ego vehicle when entering SUMO network
RealSimPara.tLookahead = 0.3; % Use 0.1 for external control, recommend tLookahead >= 0.2 for SUMO driver
RealSimPara.smoothWindow = 5; % Number of moving average data points, 1 means no moving average
% RealSimPara.speedSource = 3; % Use speedSource = 3 for dummy vehicle dynamics (simple transfer function)
RealSimPara.speedSource = 3; % Use speedSource = 2 for real vehicle dynamics (simple transfer function)
