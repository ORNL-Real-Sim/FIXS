...
from utils1 import process_spat_for_gen_speed_update
...
class SumoEnvMultiAgent:
    def __init__(self, sumoSignalConfig, traffic_layer_config, sumo_port=1337, traffic_layer_port=430, simulink_port=420, path_to_net=''):
        ...
        print(self.sumo_config_file)
        print(self.sumo_net_file)
    ...
    def subscribe_departed_veh(self):
        ...
        for veh_id in all_vehicles_unsubscribed:
            try:
    ...
    def get_travel_direction(self, road_id, route_index, route_edges, next_tls):
        # get next edge
    ...
    def start_subscription(self, mpr, step_length, vehicle_dynamics=True, eco_driving=True):
        ...
        print(f'travel direction output directory: {output_dir}')
        ego_travel_direction = 'WB'
        prv_ego_travel_direction = None
        ego_travel_direction_df = pd.DataFrame(columns=['Time', 'Direction'])
        ...
        while sim_time <= 33000:
            ...
            results_df[['travel_direction', 'control']] = results_df.apply(lambda row: self.get_travel_direction(row['road_id'], row['route_index'], row['edges'], row['next_tls']), axis=1, result_type='expand')
            ...
            # if 'ego' in results_df.index.tolist():
            #     # ego_road_id = results_df.loc['ego', 'road_id']
            #     ego_road_id = traci.vehicle.getRoadID('ego')
            #     if ego_road_id == "-2801":
            #         traci.vehicle.setRoute('ego', self.edges)
            #     if ego_road_id in [*self.wb, *self.eb]:
            #         prv_ego_travel_direction = ego_travel_direction
            #         if ego_road_id in self.wb:
            #             ego_travel_direction = 'WB'
            #         elif ego_road_id in self.eb:
            #             ego_travel_direction = 'EB'
            #     else:
            #         ego_travel_direction = prv_ego_travel_direction
            #     ego_travel_direction_df = pd.concat([ego_travel_direction_df, pd.DataFrame({'Time': [sim_time], 'Direction': [ego_travel_direction]})], ignore_index=True)
            ...
            eco_speed_dic = {key: veh.get_eco_speed_subscribe(
                                                        ...
                                                        results_df.loc[key, 'curr_status']) for index, (key, veh) in enumerate(self.cav_object_dict.items()) if key in (list_cav_control)}
            ...
            self.cav_object_dict = {key: value for key, value in self.cav_object_dict.items() if key in list_cav_control}
            traci.simulationStep()
    ...
    def apply_vehicle_control_FIXS(self, eco_speed_dic, vehicle_dynamics=True, eco_driving=True):
        ...
        for idx in range(len(self.socket_helper.vehicle_data_send_list)):
            ...
            if not vehicle_dynamics:
                ...
                print(f'Vehicle {veh_id} speed desired: {speed_desired_eco}, speed desired from simulink: {speed_desired_simulink}')
...
if __name__ == "__main__":
    ...
    exp_dir = './TestScenerio/'
    sumoSignalConfig = pd.read_csv(os.path.join(exp_dir, 'sumoSignalConfig_26.csv'), index_col=0)
    sumoSignalConfig['id'] = sumoSignalConfig['id'].astype(str)
    sumoSignalConfig['name'] = sumoSignalConfig['name'].astype(str)
    
    parser = argparse.ArgumentParser()
    parser.add_argument("-c", "--config", type=str, help="Path to the Configuration file", default=os.path.join(exp_dir, 'ecodrivingConfig.yaml'))
    ...
    parser.add_argument("--ecoDriving", action="store_true", help="Use the eco driving controller", default=None)
    parser.add_argument("--vehicleDynamics", action="store_true", help="use the vehicle dynamis", default=None)
    parser.add_argument("--penetrationRate", type=float, help="the penetration rate of cav", default=1.0)
    parser.add_argument("--pathToNet", type=str, help="the path to the net file", default=exp_dir)
