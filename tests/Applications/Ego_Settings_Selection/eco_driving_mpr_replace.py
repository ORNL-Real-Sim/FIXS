...
class SumoEnvMultiAgent:
    def __init__(self, sumoSignalConfig, sumoFolder, working_directory='MPR', replace_veh_id=None):
        # sumo startup utils
        ...
        self.replace_veh_id = replace_veh_id
    ...
    def reset(self, replace_veh_id=None):
        if replace_veh_id in traci.vehicle.getIDList():
            print('Replace vehicle {} with ego'.format(replace_veh_id))
    ...
    def change_config_directory(self, mpr, seed, step_length, subscribe):
        output_dir = os.path.join(self.sumoFolder, self.working_directory, '{}%_Seed{}_{}_{}Hz_{}'.format(int(mpr*100), seed, subscribe, int(1/step_length), self.replace_veh_id))
    ...
    def start_subscription(self, mpr, seed, step_length, replace_veh_id):
        """
...
if __name__ == "__main__":
    ...
    # red begin: 4.66 4.68
    # red mid: 4.72 4.73
    # just green (queue): 4.76 4.77 4.78 4.79
    # mid green: 4.82 4.83
    # end green: 4.84 4.85
    # about 600 s for one way trip
    target_id_list = ['4.66', '4.68', '4.72', '4.73', '4.76', '4.77', '4.78', '4.79', '4.82', '4.83', '4.84', '4.85']
    for replace_veh_id in target_id_list:
        
        start_time = time.time()
        for seed in range(100, 101):
            senv = SumoEnvMultiAgent(sumoSignalConfig, sumoFolder, replace_veh_id=replace_veh_id)

            senv.start_subscription(0.0, seed, step_length, replace_veh_id)
