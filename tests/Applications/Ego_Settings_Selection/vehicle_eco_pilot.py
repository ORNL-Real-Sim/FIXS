...
class EcoVehicle:
    ...
    def get_eco_speed_subscribe(self, phase_tracking_dict, spat_statuses, tl_light, travel_direction, orginal_desire_spd,
        ...
        if control == 'False':
            ...
            try:
                ...
                try:
                    ...
                    # orginal_desire_spd = traci.vehicle.getAllowedSpeed(self.id) * 2.23694

                    # print(t1s, t1e, t2s, t2e, r1s)
                    instant_desired_speed, mode, a_out, max_desired_speed, minimum_desired_speed = gen_desired_spd(
                        self.step_length, self.example_coasting_profile, self.A, self.B, self.C, self.M, orginal_desire_spd, self.next_movement, curr_speed_mph, curr_acc, 0,
                        dist2Stop, lead_speed_mph, lead_dist, 2, 300, 40, curr_status, t1s, t1e, t2s, t2e, r1s)

                    # follow_veh = traci.vehicle.getFollower(self.id, 100)
                    # if follow_veh[0] != '' and follow_veh[1] <= 100:
                    #     follow_id = follow_veh[0]
                    #     print('Lane Change State:', follow_id, traci.vehicle.getTypeID(follow_id),
                    #           traci.vehicle.getLaneChangeStatePretty(follow_id, 1),
                    #           traci.vehicle.getLaneChangeStatePretty(follow_id, -1),
                    #           traci.vehicle.getLaneChangeStatePretty(follow_id, 0))

                    # instant_desired_speed = curr_speed_mph + (instant_desired_speed - curr_speed_mph)/(1/self.step_length)
                    if smooth:
                        traci.vehicle.slowDown(self.id, instant_desired_speed * 0.44704, self.step_length)
                        # traci.vehicle.setAcceleration(self.id, a_out, self.step_length)
                    else:
                        traci.vehicle.setSpeed(self.id, instant_desired_speed * 0.44704)
