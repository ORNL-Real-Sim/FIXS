...
def get_traj_eval(eco_driving_folder, step_length, vtMicroCoeff, df_vehicle_src_coeff, refer_coord, direction='Both', veh_no='system', major_corridor_only=True):
    ...
    trajectory_data_eco_driving['unit_distance'] = trajectory_data_eco_driving['speed'] * step_length
    ...
    trajectory_data_energy_sum.columns = ['id', 'speed_kph_first', 'speed_kph_last', 'acceleration_mean_m_s', 'acceleration_min_m_s', 'acceleration_max_m_s',
                                          'integration_distance', 'distance_first', 'distance_last', 'distance_to_next_sum',
                                          'time_first', 'time_last', 'fuel_consume_vt_cpmf_liter', 'fuel_rate_vt_micro_liter',
                                          'trac_energy_kj', 'trac_energy_regen_kj', 'fuel_consume_yunli_icv_liter', 'energy_consume_yunli_ev_kj', 'direction']
    ...
    system_travel_dist_integration_m = trajectory_data_energy_sum['integration_distance'].sum()
    ...
    return (trajectory_data_energy_sum, system_fuel_consume_vt_cpmf_liter, system_fuel_consume_vt_micro_liter, system_energy_consume_tractive_kj,
            system_energy_consume_tractive_regen_kj, system_fuel_consume_yunli_icv_liter, system_energy_consume_yunli_ev_kj, system_travel_dist_m, system_travel_dist2_m, system_travel_dist_integration_m, system_travel_time_s)
...
def plot_distribution(energy_evaluation_summary):
...
if __name__ == "__main__":
    ...
    eco_driving_folder = r'Shallowford_after_calibration_banleftturn_AdjustedFixedTime_V3/MPR/0%_Seed100_Subscription_10Hz'
    ...
    (energy_evaluation_summary, system_fuel_consume_vt_cpmf_liter, system_fuel_consume_vt_micro_liter,
     ...
     system_energy_consume_yunli_ev_kj, system_travel_dist_m, system_travel_dist2_m, system_travel_dist_integration_m, system_travel_time_s) = \
        (get_traj_eval(eco_driving_folder, step_length, vtMicroCoeff, df_vehicle_src_coeff, refer_coord, 'Both'))

    # trajectory_data_baseline = get_traj_eval(baseline_folder, step_length)
    system_fuel_efficiency_cpmf_mpg = (system_travel_dist_m/1609.34) / (system_fuel_consume_vt_cpmf_liter/3.78541)
    system_fuel_efficiency_cpmf_mpg2 = (system_travel_dist2_m/1609.34) / (system_fuel_consume_vt_cpmf_liter/3.78541)
    system_fuel_efficiency_cpmf_mpg3 = (system_travel_dist_integration_m/1609.34) / (system_fuel_consume_vt_cpmf_liter/3.78541)

    system_fuel_efficiency_micro_mpg = (system_travel_dist_integration_m/1609.34) / (system_fuel_consume_vt_micro_liter/3.78541)
    system_fuel_efficiency_tractive_mpg = (system_travel_dist_integration_m/1609.34) / ((system_energy_consume_tractive_kj/34200)/3.78541)  # (KJ to Liter, 1 Gasoline [Liter] = 1 × 34200 = 34200 Kilojoule)
    system_fuel_efficiency_yunli_icv_mpg = (system_travel_dist_integration_m/1609.34) / (system_fuel_consume_yunli_icv_liter/3.78541)  # (KJ to Liter, 1 Gasoline [Liter] = 1 × 34200 = 34200 Kilojoule)
    system_fuel_efficiency_yunli_ev_mpg = (system_travel_dist_integration_m/1609.34) / ((system_energy_consume_yunli_ev_kj/34200)/3.78541)  # (KJ to Liter, 1 Gasoline [Liter] = 1 × 34200 = 34200 Kilojoule)

    print("\n system_fuel_consume_vt_cpmf_liter", system_fuel_consume_vt_cpmf_liter,
          "\n system_fuel_efficiency_cpmf_mpg3", system_fuel_efficiency_cpmf_mpg3, '\n',
          ...
          "\n system_travel_dist2_mile", system_travel_dist_integration_m/1609.34,
          "\n system_travel_time_h", system_travel_time_s/3600,
          "\n system_space_mean_speed_mph", (system_travel_dist_integration_m/system_travel_time_s) * 2.23694,
          "\n vehicle_counts", len(energy_evaluation_summary),"\n")

    energy_evaluation_summary.to_csv('energy_evaluation_summary_{}.csv'.format(eco_driving_folder.split('/')[-1]))
    # plot distribution
