...
def coasting_numeric_relax_sol_r(distance2stopbar, queue_coast, a_comfort, vehicle_current_speed, desired_speed, minimum_desired_speed, minimum_control_speed,
    ...
    if vehicle_current_speed > minimum_control_speed:
        ...
        a_min = ((vehicle_current_speed * 0.44704) ** 2 - (desired_speed * 0.44704) ** 2)/(2 * np.maximum((distance2stopbar - queue_coast), 1)* 0.3048)
...
def GippsSafeSpeed(step_length, acc_dist, acc_speed_base, orginal_desire_spd, minimum_gap=2):
    """
    ...
    if acc_dist >= minimum_gap:
        safe_speed = np.sqrt(2 * (acc_dist - minimum_gap) + acc_speed_base ** 2 + b ** 2 * step_length**2) - b * step_length
    else:
        safe_speed = np.sqrt(acc_speed_base ** 2 + b ** 2 * step_length ** 2) - b * step_length
...
def IntelligentDriverModel(vehicle_current_speed, orginal_desire_spd, acc_speed_base, acc_dist, minimum_gap=2, time_headway=2, step_length=1, a_max=1.5, b_comfort=2, acceleration_exp=4):
    """
    ...
    acc_dist: feet
    minimum_gap: m
    time_gap: s, desired time headway
    a_max: m/s^2
    b_comfort: m/s^2
    acceleration_exp: None
    ...
    desiredDistance = minimum_gap + np.maximum(0, (vehicle_current_speed * time_headway) +
                                               (vehicle_current_speed * (vehicle_current_speed - acc_speed_base))/(2 * np.sqrt(a_max * b_comfort)))
    ...
    instant_desired_speed = np.maximum(0, vehicle_current_speed + a_instant * step_length)

    return a_instant, instant_desired_speed * 2.23694


def IDM4LeadVehicleOrRedLight(step_length, vehicle_current_speed, orginal_desire_spd, acc_speed_base, acc_dist, distance2stopbar):

    if np.isnan(acc_dist) or np.isnan(acc_speed_base):
        a_instant, instant_desired_speed = IntelligentDriverModel(vehicle_current_speed, orginal_desire_spd, 0, distance2stopbar, step_length=step_length)
    else:
        a_instant, instant_desired_speed = IntelligentDriverModel(vehicle_current_speed, orginal_desire_spd, acc_speed_base, acc_dist, step_length=step_length)

    return a_instant, instant_desired_speed


def coasting4instant_desired_spd_toyota_v4(step_length, A, B, C, M, dist_threshold4speed_limit_1, dist_threshold4speed_limit_2,
                ...
                if desired_speed < vehicle_current_speed:
                    ...
                    if mode == 'Cruising':
                        ...
                        a_deceleration = (A + B * (vehicle_current_speed * 0.44704) + C * (
                                (vehicle_current_speed * 0.44704) ** 2)) / M  # m/s2
                        # print('status: coasting2')
                        instant_desired_speed = np.maximum(minimum_desired_speed, vehicle_current_speed - a_deceleration * 2.23694 * step_length)  # mph
                    elif mode == 'Braking':
                        instant_desired_speed = np.maximum(orginal_desire_spd * speed_limit_ratio_2, vehicle_current_speed - a_deceleration * 2.23694 * step_length)  # mph
                    elif mode == 'IDM':
                        a_instant, instant_desired_speed = IntelligentDriverModel(vehicle_current_speed, orginal_desire_spd,
                                                                                acc_speed_base, acc_dist, step_length=step_length)
                    ...
                    if vehicle_current_speed >= orginal_desire_spd * speed_limit_ratio_1:
                        ...
                        mode = 'IDM'
                        a_instant, instant_desired_speed = IntelligentDriverModel(vehicle_current_speed, np.maximum(desired_speed, 1.0),
                                                                                acc_speed_base, acc_dist, step_length=step_length)
                else:
                    mode = 'IDM'
                    a_instant, instant_desired_speed = IntelligentDriverModel(vehicle_current_speed, np.maximum(desired_speed, 1.0),
            ...
            if desired_speed == orginal_desire_spd:
                ...
                if desired_speed < vehicle_current_speed:
                    ...
                    if mode == 'Cruising':
                        ...
                        instant_desired_speed = np.maximum(desired_speed, vehicle_current_speed - a_deceleration * 2.23694 * step_length)  # mph
                    elif mode == 'IDM':
                        a_instant, instant_desired_speed = IDM4LeadVehicleOrRedLight(step_length, vehicle_current_speed, orginal_desire_spd, acc_speed_base, acc_dist, distance2stopbar)
                    elif mode == 'AEB':
                        instant_desired_speed = np.maximum(desired_speed, vehicle_current_speed - a_deceleration * 2.23694 * step_length)  # mph
                ...
                # elif vehicle_current_speed == 0:
                    ...
                    if 'red' in sig_state and vehicle_current_speed <= 10: # vehicle stop during red light
                        ...
                        mode = 'Creeping'
                        a_instant, instant_desired_speed = IDM4LeadVehicleOrRedLight(step_length, vehicle_current_speed,
                                                                                    np.maximum(desired_speed, 1.0), acc_speed_base,
    ...
    if mode == 'IDM':
        if abs(a_instant - current_acceleration) > jerk_constraint:
            if a_instant > current_acceleration:
                ...
                a_instant = current_acceleration - jerk_constraint
            instant_desired_speed = np.maximum(vehicle_current_speed + a_instant * 2.23694 * step_length, 0)
        a_out = a_instant
    elif mode == 'Cruising':
        ...
        if abs(-a_deceleration - current_acceleration) > jerk_constraint:
            if -a_deceleration > current_acceleration:
                ...
                a_deceleration = -(current_acceleration - jerk_constraint)
            instant_desired_speed = np.maximum(vehicle_current_speed - a_deceleration * 2.23694 * step_length, desired_speed)
    ...
    if np.isnan(acc_dist) or np.isnan(acc_speed_base):
        ...
        safe_speed = GippsSafeSpeed(step_length, acc_dist, acc_speed_base, orginal_desire_spd, minimum_gap=2)
...
def gen_desired_spd(step_length, example_coasting_profile, A, B, C, M, orginal_desire_spd, next_movement, vehicle_current_speed, current_acceleration, simsec, distance2stopbar,
    ...
    mode, instant_desired_speed, a_out = coasting4instant_desired_spd_toyota_v4(step_length, A, B, C, M, 2296, 1312,
                                        0.75, 0.5, vehicle_current_speed, current_acceleration,
                                        desired_speed, max(desired_speed - 1, 0), queue_coast, distance2stopbar, a_comfort, orginal_desire_spd, green_window4coast, simsec, SigState, maximum_deceleration, acc_speed_base, acc_dist, example_coasting_profile)
    ...
    #     simsec, vehicle_current_speed, queue_coast, acc_speed_base, acc_dist, instant_desired_speed, mode, a_out, desired_speed, minimum_desired_speed))
    #
    # print('------------------------------------')
    # print("\n Current Speed", vehicle_current_speed,
    #       "\n Lead Gap Distance", acc_dist,
    #       "\n Lead Veh Speed", acc_speed_base,
    #       "\n Distance to Stop", distance2stopbar,
    #       "\n Timing: ", t1s, t1e, t2s, t2e, SigState,
