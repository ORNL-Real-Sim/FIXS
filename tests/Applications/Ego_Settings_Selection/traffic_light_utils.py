...
class TrafficLight:
    ...
    def __init__(self, tlid, sumoSignalConfig):
        ...
        # sumoSignalConfig['maxDur'] = sumoSignalConfig['maxDur'] + sumoSignalConfig['yellow']

        self.tlid_phase_config = sumoSignalConfig[sumoSignalConfig['id'] == tlid][['minDur', 'maxDur', 'vehext',
                                                                 'yellow', 'red', 'name', 'movement_index_end',
        ...
        self.red_ct = {phase: self.cycle_len - (self.tlid_phase_config[phase]['maxDur'] + self.tlid_phase_config[phase]['yellow']) for phase in self.tracking_phase_list}
        self.spat_status = {}
    ...
    def get_remaining_green(self):
        ...
        for phase in self.tracking_phase_list:
            phase_status = traci.trafficlight.getRedYellowGreenState(self.tlid)[self.tlid_phase_config[phase]['movement_index_end']]
            ...
            if phase_status == 'r':
                ...
                self.red_ct[phase] = self.cycle_len - (self.tlid_phase_config[phase]['maxDur'] + self.tlid_phase_config[phase]['yellow'])
                self.initialized[phase] = True
                self.green_ct[phase] += self.step_length
                time_to_red = ((self.tlid_phase_config[phase]['maxDur'] + self.tlid_phase_config[phase]['yellow']) - self.green_ct[phase])
                # time_to_red = np.clip(((self.tlid_phase_config[phase]['maxDur'] + self.tlid_phase_config[phase]['yellow']) - self.green_ct[phase]),
                #                       0,
                #                       (self.tlid_phase_config[phase]['maxDur'] + self.tlid_phase_config[phase]['yellow']))

                if time_to_red < 0:
                    time_to_red = np.clip(time_to_red, 0, (self.tlid_phase_config[phase]['maxDur'] + self.tlid_phase_config[phase]['yellow']))
