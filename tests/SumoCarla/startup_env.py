...
def run_sumo(simulation_folder_path, simulation_net_file, port, gui=False):
    ...
    print(f'start sumo-gui -c {simulation_folder_path}\{simulation_net_name}.sumocfg --remote-port {port} --step-length 1 --netstate-dump {simulation_folder_path}\{simulation_net_name}.xml --netstate-dump.precision 10 --num-clients 1  --begin 0 --end 300')
    if gui:
        os.system(f'start sumo-gui -c {simulation_folder_path}\{simulation_net_name}.sumocfg --remote-port {port} --step-length 1 --netstate-dump {simulation_folder_path}\{simulation_net_name}.xml --netstate-dump.precision 10 --num-clients 1  --begin 0 --end 300')
    else:
        os.system(f'start sumo -c {simulation_folder_path}\{simulation_net_name}.sumocfg --remote-port {port} --step-length 1 --netstate-dump {simulation_folder_path}\{simulation_net_name}.xml --netstate-dump.precision 10 --num-clients 1  --begin 0 --end 300')
