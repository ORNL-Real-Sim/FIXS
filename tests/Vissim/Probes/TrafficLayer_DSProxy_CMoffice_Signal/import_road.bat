...
REM       TestRun SimpleTrafficLight_import (ego on the corridor-loop trajectory + Path route).
...
if not exist "%RepoRoot%	estsSumo
etworkssimple_traffic_lightsimple_traffic_light.xodr" ( echo ERROR: simple_traffic_light.xodr missing & pause & exit /b 1 )

REM --- 1. stage xodr + xosc + catalogs ------------------------------------
if not exist "%OSCDIR%" mkdir "%OSCDIR%"
copy /Y "%RepoRoot%	estsSumo
etworkssimple_traffic_lightsimple_traffic_light.xodr" "%OSCDIR%\simple_traffic_light.xodr" >nul
copy /Y "%RepoRoot%	estsSumo
etworkssimple_traffic_lightsimple_traffic_light.xosc" "%OSCDIR%\simple_traffic_light.xosc" >nul
...
"%OSC2CM%" -p "%CMPROJ%" -o "Data/OpenSCENARIO/simple_traffic_light.xosc" -r "simple_traffic_light_import" -t "SimpleTrafficLight_import" -e "Ego"
if not exist "%CMPROJ%\Data\Road\simple_traffic_light_import.rd5" ( echo ERROR: osc2cm did not produce simple_traffic_light_import.rd5 & pause & exit /b 2 )
