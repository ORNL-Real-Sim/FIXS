...
REM       TestRun (SimpleLoop_VISSIM) + ego vehicle/driver.
REM    3. Runs RealSimCarMakerSetup.py to add 20 RS_Cxxx traffic objects,
REM       producing the demo TestRun SimpleLoop_VISSIM_rs.
REM
...
"%OSC2CM%" -p "%CMPROJ%" -o "Data/OpenSCENARIO/simple_loop.xosc" -r "simple_loop" -t "SimpleLoop_VISSIM" -e "Ego"
...
if not exist "%PYTHON%" (
    ...
    echo   The base TestRun SimpleLoop_VISSIM exists but has NO traffic objects;
...
"%PYTHON%" "%RepoRoot%\CarMaker\RealSimCarMakerSetup.py" --cm-project-path "%CMPROJ%" --testrun "SimpleLoop_VISSIM" --cm-install-path "C:\IPG" --output-testrun "SimpleLoop_VISSIM_rs" --car 20 --truck 0 --no-random-traffic

echo.
echo === Import complete ===
echo   Road:    %CMPROJ%\Data\Road\simple_loop.rd5
echo   TestRun: %CMPROJ%\Data\TestRun\SimpleLoop_VISSIM_rs  (ego + 20 traffic objects)
