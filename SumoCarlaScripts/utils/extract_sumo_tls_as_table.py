...
def turn_dir_rank(turn_dir):
    """Sort signal heads left-to-right from the driver's perspective."""
    turn_dir = (turn_dir or '').lower()
    if turn_dir == 'l':
        return 0
    if turn_dir in ('s', 't'):
        return 1
...
def parse_sumo_tls_from_netxml(
    ...
    #    build: tls_links[tls_id][linkIndex] -> list of (inLane, outLane, viaLane, turnDir, connSeq)
    tls_links = {}
    conn_seq = 0
    for conn in root.iter('connection'):
        ...
        tls_links[tls_id][int(link_idx)].append((in_lane, out_lane, via_lane, turn_dir, conn_seq))
        conn_seq += 1

    # 4) Build groups & compute positions
    traffic_light_groups = {}
    in_lane_links_total = {}
    in_lane_offset_index = {}

    # Pre-order links per inLane so right-turn heads land on the driver's right.
    in_lane_links = {}
    for _, linkIndexGroups in tls_links.items():
        for link_idx, triplets in linkIndexGroups.items():
            for inLane, _, _, turn_dir, seq in triplets:
                in_lane_links.setdefault(inLane, []).append(
                    (turn_dir_rank(turn_dir), link_idx, seq)
                )
    for inLane, entries in in_lane_links.items():
        ordered = sorted(entries)
        in_lane_links_total[inLane] = len(ordered)
        for offset_idx, (_, _, seq) in enumerate(ordered):
            in_lane_offset_index[seq] = offset_idx
    ...
    for tls_id, linkIndexGroups in tls_links.items():
        ...
        for link_idx, triplets in sorted(linkIndexGroups.items()):
            for inLane, outLane, viaLane, _, seq in triplets:
                ...
                junction_shift_x = 0.0
                junction_shift_y = 0.0

                # Build the signal-head group at the incoming-side junction boundary,
                # then shift the whole group to the opposite side of the junction.
                if inter_points:
                    end_pt = Point(shape_xy[-1])
                    entry_pt = min(inter_points, key=lambda p: Point(p).distance(end_pt))
                    exit_pt = max(inter_points, key=lambda p: Point(p).distance(end_pt))
                    base_x, base_y = entry_pt
                    junction_shift_x = exit_pt[0] - entry_pt[0]
                    junction_shift_y = exit_pt[1] - entry_pt[1]
                ...
                # Cluster multiple signal heads from the same inLane next to each other.
                L = in_lane_links_total.get(inLane, 1)
                i = in_lane_offset_index.get(seq, 0)
                cluster_width = signal_head_spacing * max(L - 1, 0)
                d_perp = (i * signal_head_spacing) - (cluster_width / 2.0) if apply_linkwise_offset else 0.0

                # forward offset along the tangent
                bx = offset_forward * tx
                by = -offset_forward * ty

                px = base_x + junction_shift_x + bx + d_perp * nx
                py = base_y + junction_shift_y + by + d_perp * ny
...
if __name__ == "__main__":
    sumo_net_file = '../roosevelt/Roosevelt_SUMOfile/roosevelt.net.xml'
    output_path = '../roosevelt/tl_table'
